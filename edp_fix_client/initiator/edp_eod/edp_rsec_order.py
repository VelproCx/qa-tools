        ...
        for data_dict in self.data_list:
            for field in fields:
                if not data_dict[field]:
                    errors.append('{} not inconsistency,OrderID:{}'.format(field, data_dict["OrderID"]))
            ...
            #     if not data_dict["Account"].strip() and 'R{}_EDP_ACCOUNT_'.format(self.env) not in data_dict["Account"]:
